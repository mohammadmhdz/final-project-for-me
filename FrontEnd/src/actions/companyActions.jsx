...
export const companyListAction = (keyword) => async (dispatch) => {
  try {
...
export const companyDetails = (keyword) => async (dispatch) => {
  ...
  try {
    ...
    const { data } = await axios.get(
      `http://127.0.0.1:8000/api/companies/${keyword}`
    );
    dispatch({
...
export const updatecompanyDetailss = (input) => async (dispatch) => {
  try {
    dispatch({
      type: COMPANY_DETAILS_UPDATE_REQUEST,
    });
    ...
    const { data } = await axios.put(
      `http://127.0.0.1:8000/api/companiess/${input.id}/`,
      {
        Name: input.Name,
        Owner_name: input.Owner_name,
        active_plan: input.active_plan,
